...
def parse_state_bits(raw_state: str) -> Optional[int]:
    """Parse a numeric task state value from crash output."""
    match = re.search(r"0x[0-9a-fA-F]+|\d+", raw_state)
    if not match:
        return None
...
def format_task_struct(data: Dict[str, str]) -> str:
    ...
    if "state" in data:
        ...
        output.append("-" * 80)
        state_bits = parse_state_bits(state_val)
        if state_bits is None:
            output.append(f"  → Unparsed raw state: {state_val}")
        elif state_bits == 0:
            output.append("  → TASK_RUNNING")
        else:
            states = [
            ...
            matched = False
            for bit, name in states:
                if state_bits & bit:
                    output.append(f"  → {name}")
                    matched = True
            if not matched:
                output.append(f"  → Unknown state bits: {state_bits}")
...
def format_backtrace(frames: List[Tuple[int, str, str]]) -> str:
    ...
    for frame_num, address, function in frames:
        ...
        if any(keyword in function.lower() for keyword in ["panic", "oops", "bug", "warn", "error", "fault"]):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze kernel data structures from crash output")
    parser.add_argument("input_file", help="File containing crash command output")
    parser.add_argument(
        "--type",
