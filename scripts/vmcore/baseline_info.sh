...
# 使用：bash baseline_info.sh <vmcore_path> <vmlinux_path> [src_dir]
# 参数：
...
BRANCH_MAP["NULL pointer dereference|unable to handle kernel NULL"]="分支A: 空指针解引用    → scripts/vmcore/branches/branch_A_null_ptr.sh"
BRANCH_MAP["KASAN: slab-out-of-bounds|KASAN.*out-of-bounds"]="分支B: 内存越界OOB      → scripts/vmcore/branches/branch_B_oob.sh"
BRANCH_MAP["KASAN: use-after-free|use-after-free"]="分支C: Use-After-Free   → scripts/vmcore/branches/branch_C_uaf.sh"
BRANCH_MAP["stack-protector|stack overflow|stack guard"]="分支D: 内核栈溢出       → scripts/vmcore/branches/branch_D_stack_overflow.sh"
BRANCH_MAP["Machine check:|mce.*bank|MCE.*PROCESSOR"]="分支E: 硬件MCE          → scripts/vmcore/branches/branch_E_mce.sh"
BRANCH_MAP["EDAC.*UE|uncorrectable.*error"]="分支F: 内存UE           → scripts/vmcore/branches/branch_F_memory_ue.sh"
BRANCH_MAP["possible circular locking|LOCKDEP.*circular"]="分支G: 死锁             → scripts/vmcore/branches/branch_G_deadlock.sh"
BRANCH_MAP["soft lockup|softlockup"]="分支H: Soft Lockup      → scripts/vmcore/branches/branch_H_soft_lockup.sh"
BRANCH_MAP["hard LOCKUP|NMI watchdog.*LOCKUP"]="分支I: Hard Lockup      → scripts/vmcore/branches/branch_I_hard_lockup.sh"
BRANCH_MAP["kernel BUG at|BUG.*line"]="分支J: BUG()触发        → scripts/vmcore/branches/branch_J_bug_trigger.sh"
BRANCH_MAP["Out of memory|oom_kill|Killed process"]="分支K: OOM Killer       → scripts/vmcore/branches/branch_K_oom.sh"
BRANCH_MAP["sleeping function called from invalid|might sleep"]="分支L: 原子上下文睡眠   → scripts/vmcore/branches/branch_L_atomic_sleep.sh"
BRANCH_MAP["rcu_sched detected stalls|RCU stall"]="分支M: RCU Stall        → scripts/vmcore/branches/branch_M_rcu_stall.sh"
BRANCH_MAP["EXT4-fs error|XFS.*corruption|btrfs.*corrupt"]="分支N: 文件系统崩溃     → scripts/vmcore/branches/branch_N_fs_corruption.sh"
BRANCH_MAP["double free.*skb|kfree_skb.*double|skb.*poison"]="分支O: 网络子系统崩溃   → scripts/vmcore/branches/branch_O_network.sh"
BRANCH_MAP["DMA mapping error|I/O timeout.*abort|blk.*abort"]="分支P: 存储IO崩溃       → scripts/vmcore/branches/branch_P_storage_io.sh"
BRANCH_MAP["vmx_exit|kvm_.*exit|VMX.*exit reason"]="分支Q: KVM/vCPU异常     → scripts/vmcore/branches/branch_Q_kvm.sh"
BRANCH_MAP["acpi_.*error|AE_BAD_ADDRESS|ACPI Error"]="分支R: ACPI固件异常     → scripts/vmcore/branches/branch_R_acpi.sh"
BRANCH_MAP["migrate_pages.*fail|offline_pages.*error|page migration"]="分支S: 热插拔/页迁移    → scripts/vmcore/branches/branch_S_hotplug.sh"
BRANCH_MAP["memory_failure|hwpoison|HardwareCorrupted"]="分支F+V: 内存硬件故障   → scripts/vmcore/branches/branch_F_memory_ue.sh / branch_V_bit_flip.sh"
...
BIT_FLIP_LABEL="分支V: 疑似Bit Flip → scripts/vmcore/branches/branch_V_bit_flip.sh"
