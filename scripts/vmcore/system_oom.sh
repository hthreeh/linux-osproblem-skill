...
cmd_info \
    "dmesg -T + 锚定 'Out of memory: Kill' 触发行，捕获到下一个 OOM 事件前" \
    "内核 OOM 输出顺序：[1]触发行 → [2]task list（数千行） → [3]Mem-Info（数百行） → [4]Killed process" \
    "从触发行[1]开始捕获，自然包含后续所有[2][3][4]，完整呈现 OOM 触发时的内存快照和进程列表"

# 只锚定 "Out of memory: Kill" 触发行，捕获整个 OOM 事件块
# 内核 OOM 输出顺序：[1]触发行 → [2]task list → [3]Mem-Info → [4]Killed process
# 从触发行 [1] 开始捕获，自然包含后续所有 [2][3][4] 内容
grep -n "Out of memory: Kill\|oom_kill_process" "$DMESG_TMP" 2>/dev/null \
    | head -10 \
    | while IFS=: read -r anchor_lineno rest; do
        echo ""
        echo ">>> OOM 事件（触发行: dmesg 行 $anchor_lineno）<<<"

        # 从触发锚点行开始，捕获到下一个 "Out of memory" 之前的所有行
        # 这样包含 task list + Mem-Info + Killed process 的完整事件块
        next_anchor=$(grep -n "Out of memory: Kill\|oom_kill_process" "$DMESG_TMP" 2>/dev/null \
            | awk -F: -v start="$anchor_lineno" '$1 > start {print $1; exit}')

        total_lines=$(wc -l < "$DMESG_TMP" 2>/dev/null || echo 0)
        if [ -n "$next_anchor" ]; then
            end_lineno=$((next_anchor - 1))
        else
            end_lineno=$total_lines
        fi

        captured=$((end_lineno - anchor_lineno + 1))
        if [ "$captured" -gt 5000 ]; then
            # 进程列表可能非常大，限制输出以防脚本挂起
            echo "  [完整输出共 $captured 行，显示前 3000 行（task list + Mem-Info + Killed process）...]"
            sed -n "${anchor_lineno},$((anchor_lineno + 2999))p" "$DMESG_TMP"
            echo "  [... 后续 $((captured - 3000)) 行省略，完整数据见 $OUTPUT_DIR/dmesg_tmp.txt ...]"
        else
            sed -n "${anchor_lineno},${end_lineno}p" "$DMESG_TMP"
        fi
    done
